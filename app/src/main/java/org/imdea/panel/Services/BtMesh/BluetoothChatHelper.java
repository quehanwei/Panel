...
public class BluetoothChatHelper {
     ...
     */
    public synchronized void connect(Device device, boolean secure) {
     ...
     */
    public synchronized void connected(BluetoothSocket socket, Device
     ...
     * @see BluetoothChatHelper.ConnectedThread#write(byte[])
     */
                    ...
                    synchronized (BluetoothChatHelper.this) {
                        switch (mState) {
                            ...
                            case STATE_CONNECTING:
                                ...
                                connected(socket, mContext.getDevice(socket.getRemoteDevice()), mSocketType);
                                break;
                            case STATE_NONE:
    ...
    private class ConnectThread extends Thread {
        ...
        public ConnectThread(Device device, boolean secure) {
            mmDevice = device;
            ...
            try {
                if (secure) {
                    tmp = device.btDevice.createRfcommSocketToServiceRecord(
                            MY_UUID_SECURE);
                } else {
                    tmp = device.btDevice.createInsecureRfcommSocketToServiceRecord(
                            MY_UUID_INSECURE);
                }
            } catch (IOException e) {
